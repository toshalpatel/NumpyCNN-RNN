...
class BiRNN(Layer):
    ...
    def forward(self, input):
        ...
        h_rev = self.backward_rnn.forward(self._reverse_temporal_data(input,mask))
        h_rev_out = self._reverse_temporal_data(h_rev,mask) 
        output = np.concatenate([h, h_rev_out],axis=2)
